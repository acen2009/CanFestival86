...
bool CanFestival86::sdo(UNS8 nodeId, UNS16 index, UNS8 subIndex, UNS32 size, void *data, SDOCallback_t Callback){
    if(_CO_Data == NULL){
        return false;
    }
    if(writeNetworkDictCallBack(_CO_Data, nodeId, index, subIndex, size, 0, data, Callback, 0) != 0){
        Serial.println("Something wrong while sending a SDO request");
        return false;
    }
    return true;
}
...
void sdoDefaultCallback(CO_Data *d, uint8_t nodeId){
    ...
    if(getWriteResultNetworkDict (d, nodeId, &abortCode) != SDO_FINISHED){        
        err_print("SDO Failed\n");
    }
    else{        
        err_print("SDO Succeeded\n");
    }
