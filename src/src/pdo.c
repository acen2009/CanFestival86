	...
	if (offset && RPDOIndex >= 0x1400)
		{
			...
			if (offset <= lastIndex)
				{
					/* get the CobId */
					pwCobId = (uint16_t*)d->objdict[offset].pSubindex[1].pObject;
	...
	{ 
		...
		if (offsetObjdict)
			while (offsetObjdict <= lastIndex)
			{
				...
				{
				case state2:
					pwCobId = (uint16_t*)d->objdict[offsetObjdict].pSubindex[1].pObject;
...
UNS8 _sendPDOevent (CO_Data * d, UNS8 isSyncEvent)
{
	...
	if (offsetObjdict)
	{
		...
		while (offsetObjdict <= lastIndex)
		{
			...
			{
			case state3:
				/* bSubCount always 5 with objdictedit -> check disabled */
				...
				else if (isSyncEvent && (*pTransmissionType == TRANS_RTR_SYNC))
				{
					...
					/* If transmission on Event and not inhibited, check for changes */
						  ...
						  (!isSyncEvent && (*pTransmissionType == TRANS_EVENT_PROFILE || 
										    *pTransmissionType == TRANS_EVENT_SPECIFIC) && !(d->PDO_status[pdoNum].transmit_type_parameter & PDO_INHIBITED)))
				{
