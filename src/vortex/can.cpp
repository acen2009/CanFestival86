/*
  can.cpp - DM&P Vortex86 CAN Bus library
...
DMP_INLINE(bool) can_RoundRobin(CAN_Bus *can)
{
	CANFrame temp;
	...
	if (io_In32(can->ioHandle, can->REQ) & (0x01L << (can->round*2)))
		return false;
	if (io_In32(can->ioHandle, can->ISR) & (CAN_TX0I << can->round))
		return false;
	...
	io_Out32(can->ioHandle, can->REQ, 0x01UL << (can->round*2));
...
volatile unsigned long can_isr_cnt = 0;
volatile unsigned long can_isr_rcv = 0;
volatile unsigned long can_isr_trx = 0;
volatile unsigned long can_isr_trxerr = 0;
volatile unsigned long can_isr_trxsucc = 0;
volatile unsigned long can_isr_trxfail = 0;
...
static int CAN_ISR(int irq, void *data)
{
	...
	isr = io_In32(can->ioHandle, can->ISR);
	if (!(isr & 0x07FFL))
		return ISR_NONE;

	/* Must clear RX interrupt after release message. */
	io_Out32(can->ioHandle, can->ISR, isr & ~CAN_RXI);
	...
	if (isr & CAN_RXI)
	{
		...
		if (!(rx_temp.type & STD_REMOTE_FRAME)) {
			rx_temp.Data.dword[0] = io_In32(can->ioHandle, can->RX.DATAL);
			rx_temp.Data.dword[1] = io_In32(can->ioHandle, can->RX.DATAH);
			for (i = rx_temp.length; i < 8; i++) {
				rx_temp.Data.byte[i] = 0x00;
			}
		} else {
			rx_temp.Data.dword[0] = 0x00000000UL;
			rx_temp.Data.dword[1] = 0x00000000UL;
		}
	...
	if (isr & CAN_TX0I)
	{
		...
		if((stat & 0x07UL) == 0x05UL){
			can_isr_trxsucc++;
		}
		else if((stat & 0x07UL) == 0x00UL){
			can_isr_trxfail++;
		}
	}
	...
	while (txcnt-- > 0 && can_RoundRobin(can));

	return ISR_HANDLED;
}

DMPAPI(void *) CreateCANBus(int canNo, int IO_Space)
{
	CAN_Bus *can;
	
	if (canNo != CAN0 && canNo != CAN1) {
		err_print((char*)"%s: Wrong CAN number.\n", __FUNCTION__);
		return NULL;
	}
	
	if (IO_Space != IO_PORT && IO_Space != IO_MEMORY) {
		err_print((char*)"%s: Wrong IO space.\n", __FUNCTION__);
		return NULL;
	}
...
DMPAPI(bool) can_Init(void *vcan)
{
	...
	switch (can->IO_Space)
	{
		case IO_PORT:
		{
			...
			printf("Use Port I/O space: 0x%04x\n", (unsigned short)baseAddress);
		}
		break;
		...
		case IO_MEMORY:
		{
			...
			printf("Use Memory space: 0x%08lx\n", baseAddress);
		}
		break;
	...
	printf("IRQ No. %d\n\n", can->nIRQ);
	io_DisableINT();
	irq_Setting(can->nIRQ, IRQ_LEVEL_TRIGGER + IRQ_DISABLE_INTR);
															...
															, unsigned int ps2)
{
	...
	/* CAN bus clock source using external clock. */
	cps = 0x80000000UL;
	
	/* Check CAN bus clock divider */
	if (ckdiv > 65535 || ckdiv < 2 || (ckdiv%2) != 0) {
		err_print((char*)"%s: 'ckdiv' error.\n", __FUNCTION__);
		return false;
	}
	cps |= (unsigned long)((ckdiv >> 1) - 1);
	
	/* CAN bus bit-time is sampled once at sample point. */
	btr = 0x0000UL;
...
{
	unsigned int ckdiv, sjw, prog, ps1, ps2;
	unsigned long info;
	CAN_Bus *can = (CAN_Bus *)vcan;
	...
	info = vx86_GetCANBusBpsInfo(bps);
	if (info == 0xFFFFFFFFUL) {
		err_print((char*)"%s: Unsupported CAN bus bit rate.\n", __FUNCTION__);
		return false;
	}

	/* Get CAN bus clock divider */
	ckdiv = (unsigned int)((info >> 16) & 0xFFUL);
	
	/* Get CAN bus synchronization jump width */
	sjw = (unsigned int)((info >> 12) & 0x0FUL);
	
	/* Get CAN bus propagation segment */
	prog = (unsigned int)((info >> 8) & 0x0FUL);
	
	/* Get CAN bus phase segment 1 */
	ps1 = (unsigned int)((info >> 4) & 0x0FUL);
	
	/* Get CAN bus phase segment 2 */
	ps2 = (unsigned int)(info & 0x0FUL);
	
	return can_SetBitTiming(vcan, ckdiv, sjw, prog, ps1, ps2);
}
